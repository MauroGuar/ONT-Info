...
def ipv4_validation(ip):
    partes = ip.split('.')
    if len(partes) != 4:
        error_return("La ip introducida no esta compuesta por 4 partes divididas por un '.'",
    ...
    try:
        for parte in partes:
            ...
            if len(parte) > 1 and parte[0] == '0':
                error_return("La ip introducida contiene 0s a la izquierda o de relleno, formato no permitido.",
                             "The ip contains zero padding/leading zeroes (like the zero in 013), incorrect format")
            return ip
    except ValueError:
        error_return("La ip introducida contiene caracteres que no son enteros, verifique por errores de syntaxis ",
                     "parseint failed")
...
def sn_validation(introduced_serial, introduced_length, introduced_base, introduced_dictionary={'Ñ': 'N'}):
    ...
    try:
        ...
        if len(serial) != introduced_length:
            error_return(f"El sn introducido no satisface la longitud requerida: {introduced_length} caracteres",
